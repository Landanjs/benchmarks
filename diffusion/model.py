...
class Diffusion(nn.Module):
    ...
    def forward_process(self, imgs, time_steps):
        ...
        # Timesteps range for 1 to T since 0 means no noise, so subtract 1
        extra_dims = (1,) * (imgs.dim() - 1) # Extra dims required to broadcast 1D tensor with image
        img_scaled = self.sqrt_alphas_cumprod[time_steps - 1].view(-1, *extra_dims) * imgs

        noise = torch.randn_like(imgs)
        noise_scaled = self.sqrt_one_minus_alphas_cumprod[time_steps - 1].view(-1, *extra_dims) * noise
...
class ComposerDDPMUnet(ComposerModel):
    ...
    def __init__(self, dim, T = 1000):
        ...
        self.train_metrics = MetricCollection([MeanSquaredError()])
    ...
    def forward(self, batch):
        imgs = batch[0]
        # Sample times for training uniformly (I don't understand the math of why this works... I should though)
    ...
    def loss(self, forward_out, batch):
        return F.mse_loss(forward_out[0], forward_out[1])
